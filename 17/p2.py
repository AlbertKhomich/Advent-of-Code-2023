...
while queue:
    ...
    if (row, col) == (len(grid) - 1, len(grid[0]) - 1) and steps >= 4:
        print(heat)
        break
    ...
    if steps >= 4 or (dir_row, dir_col) == (0, 0):
            ...
            if ((next_dir_row, next_dir_col) != (dir_row, dir_col) and
               (next_dir_row, next_dir_col) != (-dir_row, -dir_col)):
                ...
                if 0 <= next_row < len(grid) and 0 <= next_col < len(grid[0]):
                    heappush(
                        queue,
                        (heat + grid[next_row][next_col], next_row, next_col, next_dir_row, next_dir_col, 1)
                    )
